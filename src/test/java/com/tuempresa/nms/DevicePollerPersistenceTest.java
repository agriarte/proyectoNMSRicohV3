 ...
 *
 * A diferencia de DevicePollerIntegrationTest (que usa repositorios en
 * memoria), este test SI valida el mapeo JPA, el DDL generado por Hibernate y
 * la conexion real a Postgres.
 */
@Testcontainers
@SpringBootTest
class DevicePollerPersistenceTest {
	...
	static PostgreSQLContainer<?> postgres = new PostgreSQLContainer<>("postgres:16-alpine")
			.withDatabaseName("nms_printers_test").withUsername("test_user").withPassword("test_pass");
	 ...
	 * por el mock ya usado en DevicePollerIntegrationTest. Todo lo demas (JPA,
	 * repositorios, RicohDriver, DriverRegistry) es el bean real de Spring.
	 */
	@TestConfiguration
	...
	void poll_ricohImC5510_persistsDeviceAndReadingInRealPostgres() {
		...
		List<ReadingEntity> readings = readingRepository.findByDeviceIdOrderByPolledAtDesc(savedDevice.get().getId());
